...
export default function LuxePLPPage({
  products,
  categories,
  brands,
  filterFacets,
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--sf-bg)' }}>
      ...
      <main className="flex-1">
        ...
        >
          ...
          <p
            className="mt-3 text-sm font-light max-w-xl mx-auto"
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-6 text-xs tracking-[0.15em] uppercase font-light">
              <div className="flex gap-3 items-center">
                <span style={{ color: 'color-mix(in srgb, var(--sf-text) 30%, transparent)' }}>
                  Gender:
                </span>
                <button
                  onClick={() => updateParams({ gender: undefined })}
                  style={{
                    color: !activeGender ? 'var(--sf-primary)' : 'var(--sf-text)',
          ...
          <div
            className="flex flex-wrap items-center justify-between gap-4 mb-10 pb-4 border-b"
            style={{ borderColor: 'color-mix(in srgb, var(--sf-text) 6%, transparent)' }}
          >
            {availableBrands.length > 0 && (
              <div className="flex flex-wrap gap-2 items-center">
                <span
                  className="text-[11px] tracking-[0.15em] uppercase font-light mr-1"
                  ...
                  <button
                    ...
                    style={{
                      color:
                        activeBrand === (brand.slug || brand.name)
                          ? 'white'
                          : 'color-mix(in srgb, var(--sf-text) 60%, transparent)',
                      backgroundColor:
                        activeBrand === (brand.slug || brand.name)
                          ? 'var(--sf-primary)'
                          : 'transparent',
            ...
            <div className="flex gap-2 items-center text-xs tracking-wider">
              ...
              {discountOptions.map((d) => (
                <button
                  key={d.value}
                  onClick={() =>
                    updateParams({ discount: activeDiscount === d.value ? undefined : d.value })
                  }
                  className="px-2.5 py-1 text-[11px] font-light border"
                  style={{
                    backgroundColor:
                      activeDiscount === d.value ? 'var(--sf-primary)' : 'transparent',
              ...
              {hasFilters && (
                <button
                  ...
                  className="text-xs tracking-widest uppercase ml-4 underline text-rose-500 font-light"
                >
                  Reset
                </button>
              )}
