...
function StorefrontImageSlider({ block }: { block: any }) {
  ...
  return (
    <div className="relative rounded-3xl overflow-hidden shadow-xl" style={{ height: block.data?.height || '520px' }}>
      <div
      ...
      <div className={`relative z-10 h-full flex flex-col justify-center p-8 sm:p-16 text-white text-${activeSlide.textAlign || 'center'} items-${activeSlide.textAlign === 'left' ? 'start' : activeSlide.textAlign === 'right' ? 'end' : 'center'}`}>
        <div className="max-w-2xl space-y-5">
          ...
          <div className={`pt-2 flex flex-wrap gap-3 justify-${activeSlide.textAlign === 'left' ? 'start' : activeSlide.textAlign === 'right' ? 'end' : 'center'}`}>
            {activeSlide.buttonText && (
              <Link href={activeSlide.buttonUrl || '/products'} className="px-7 py-3.5 rounded-xl bg-white text-slate-950 font-bold text-sm shadow-xl hover:bg-slate-100 transition">
                {activeSlide.buttonText}
              </Link>
            )}
            {activeSlide.secondaryButtonText && (
              <Link href={activeSlide.secondaryButtonUrl || '/collections'} className="px-7 py-3.5 rounded-xl bg-white/20 text-white font-bold text-sm backdrop-blur-md hover:bg-white/30 transition">
                {activeSlide.secondaryButtonText}
              </Link>
            )}
          </div>
...
function StorefrontCountdown({ block }: { block: any }) {
  ...
  return (
    <div className={`p-8 sm:p-12 rounded-3xl bg-gradient-to-r ${data?.bgGradient || 'from-violet-950 via-indigo-900 to-slate-950'} text-white space-y-6 shadow-2xl`}>
      <div className="text-center space-y-2">
        {data?.badge && <span className="inline-block px-3 py-1 rounded-full bg-amber-400 text-slate-950 font-black text-xs uppercase tracking-wider">{data.badge}</span>}
        <h3 className="text-2xl sm:text-4xl font-black font-heading">{data?.title || 'Flash Sale'}</h3>
        {data?.subtitle && <p className="text-sm text-slate-300 max-w-md mx-auto">{data.subtitle}</p>}
      </div>
      ...
      <div className="flex justify-center gap-3 sm:gap-4 py-2">
        {[
          ...
          <div key={idx} className="w-20 sm:w-24 p-3.5 sm:p-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-center">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
        ...
        {data?.buttonText && (
          <Link href={data.buttonUrl || '/products'} className="px-8 py-3.5 rounded-xl bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs shadow-xl transition">
            {data.buttonText}
          </Link>
        )}
      </div>
            ...
            <div className="space-y-12">
              {parsed.map((block: any, idx: number) => {
                ...
                switch (block.type) {
                  // ── 1. HEADING TAG (H1-H6) ──
                  case 'heading': {
                    const Tag = (['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(block.data?.level) ? block.data.level : 'h2') as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';
                    const sizeCls = {
                      xl: 'text-xl',
                    ...
                    return (
                      <div key={block.id || idx} className={`py-4 text-${block.data?.textAlign || 'center'} space-y-2`}>
                        {block.data?.eyebrow && (
                          <div className="inline-block px-3 py-1 rounded-full bg-slate-100 font-extrabold text-[11px] tracking-wider uppercase mb-1" style={{ color: 'var(--sf-primary)' }}>
                            {block.data.eyebrow}
                          </div>
                        )}
                        <Tag
                        ...
                        {block.data?.hasUnderline && (
                          <div className={`w-16 h-1 rounded-full mt-3 ${block.data?.textAlign === 'left' ? '' : block.data?.textAlign === 'right' ? 'ml-auto' : 'mx-auto'}`} style={{ backgroundColor: 'var(--sf-primary)' }} />
                        )}
                      </div>
                    );
                  }
                  ...
                  case 'span_badge': {
                    ...
                    const colorCls = colorClasses[block.data?.colorPreset || 'violet'] || colorClasses.violet;
                    const shapeCls = shapeClasses[block.data?.variant || 'pill'] || shapeClasses.pill;

                    const content = (
                      <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-black tracking-wide uppercase border shadow-xs ${colorCls} ${shapeCls}`}>
                    ...
                    return (
                      <div key={block.id || idx} className={`flex justify-${block.data?.align === 'left' ? 'start' : block.data?.align === 'right' ? 'end' : 'center'}`}>
                        {block.data?.linkUrl ? (
                      ...
                      <div key={block.id || idx} className="flex justify-center">
                        <div
                          ...
                          style={{
                            ...
                            borderStyle: block.data?.borderStyle === 'none' ? 'none' : block.data?.borderStyle || 'solid',
                          }}
                        >
                          {block.data?.title && <h3 className="text-lg font-bold font-heading text-slate-900 mb-2">{block.data.title}</h3>}
                          <div className="prose prose-slate max-w-none" dangerouslySetInnerHTML={{ __html: block.data?.content || '' }} />
                        </div>
                      </div>
                    );
                  }
                  ...
                  case 'paragraph': {
                    return (
                      <div key={block.id || idx} className={`text-${block.data?.textAlign || 'center'} px-4`}>
                        <p className={`text-${block.data?.fontSize || 'base'} text-slate-600 dark:text-slate-300 leading-${block.data?.lineHeight || 'relaxed'} max-w-3xl mx-auto font-medium`}>
                          {block.data?.text || ''}
                        </p>
                      </div>
                    );
                  ...
                  case 'button_group': {
                    return (
                      <div key={block.id || idx} className={`flex flex-wrap gap-3 justify-${block.data?.align === 'left' ? 'start' : block.data?.align === 'right' ? 'end' : 'center'}`}>
                        {(block.data?.buttons || []).map((b: any, bIdx: number) => {
                          ...
                          return (
                            <Link key={bIdx} href={b.url || '/products'} className={`inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-bold text-sm transition ${styleClass}`}>
                  ...
                  case 'divider_spacer': {
                    return (
                      <div key={block.id || idx} className="flex items-center justify-center relative my-2" style={{ height: `${block.data?.height || 48}px` }}>
                  ...
                  case 'product_slider': {
                    return (
                      <div key={block.id || idx} className="p-8 space-y-6 bg-slate-50/70 dark:bg-neutral-900 rounded-3xl border border-slate-200/80 dark:border-neutral-800 shadow-sm">
                        <div className="flex items-end justify-between">
                          <div>
                            <h3 className="text-2xl font-black font-heading text-slate-900 dark:text-white">{block.data?.heading || 'Trending Bestsellers'}</h3>
                            {block.data?.subtitle && <p className="text-xs text-slate-500 mt-1">{block.data.subtitle}</p>}
                          </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
                          {(block.data?.items || []).map((p: any, pIdx: number) => (
                            <Link key={pIdx} href={p.url || '/products'} className="group rounded-2xl border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 overflow-hidden shadow-xs hover:shadow-md transition">
                              <div className="relative aspect-square bg-slate-100 dark:bg-neutral-900 overflow-hidden">
                                {p.image ? <img src={p.image} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" /> : null}
                                {p.badge && <span className="absolute top-2.5 left-2.5 px-2 py-0.5 rounded-md bg-slate-900 text-white font-black text-[10px] uppercase tracking-wider">{p.badge}</span>}
                              </div>
                              <div className="p-4 space-y-1">
                                ...
                                <div className="flex items-center gap-2 pt-1">
                                  ...
                                  {p.compareAtPrice && <span className="text-xs text-slate-400 line-through">{p.compareAtPrice}</span>}
                                </div>
                              </div>
                            </Link>
                          ))}
                  ...
                  case 'announcement_bar': {
                    return (
                      <div key={block.id || idx} className="p-3.5 rounded-2xl bg-indigo-950 text-white flex flex-wrap items-center justify-center gap-3 text-xs sm:text-sm shadow-md">
                        {block.data?.badge && <span className="px-2.5 py-0.5 rounded-full bg-amber-400 text-slate-950 font-black text-[10px] uppercase">{block.data.badge}</span>}
                        <span className="font-bold">{block.data?.message}</span>
                        {block.data?.couponCode && <span className="px-2.5 py-0.5 rounded-md bg-white/20 font-mono font-black text-amber-300">{block.data.couponCode}</span>}
                        {block.data?.ctaText && (
                          <Link href={block.data.ctaUrl || '/products'} className="underline font-bold text-amber-300 ml-1 hover:text-white">
                            {block.data.ctaText} →
                          </Link>
                        )}
                      </div>
                  ...
                  case 'pricing_table': {
                    return (
                      <div key={block.id || idx} className="space-y-8 py-4">
                        <div className="text-center space-y-2">
                          <h3 className="text-3xl font-black font-heading text-slate-900 dark:text-white">{block.data?.heading || 'Membership Plans'}</h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-stretch max-w-6xl mx-auto">
                          {(block.data?.plans || []).map((p: any, pIdx: number) => (
                            <div key={pIdx} className={`p-8 rounded-3xl border flex flex-col justify-between ${p.isPopular ? 'border-indigo-600 bg-indigo-50/20 shadow-2xl relative' : 'border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900'}`}>
                              {p.isPopular && <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-indigo-600 text-white font-black text-[10px] uppercase tracking-wider">POPULAR CHOICE</span>}
                              ...
                              <Link href={p.buttonUrl || '/checkout'} className={`w-full mt-8 py-3.5 rounded-xl font-bold text-xs text-center block transition ${p.isPopular ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' : 'bg-slate-900 dark:bg-white text-white dark:text-black hover:opacity-90'}`}>
                  ...
                  case 'trust_badges': {
                    return (
                      <div key={block.id || idx} className="p-8 rounded-3xl bg-slate-50 dark:bg-neutral-900 border border-slate-200/80 dark:border-neutral-800 space-y-6">
                        {block.data?.heading && <h4 className="text-sm font-black text-center text-slate-900 dark:text-white uppercase tracking-wider">{block.data.heading}</h4>}
                  ...
                  case 'banner_grid': {
                    return (
                      <div key={block.id || idx} className="space-y-6">
                        {block.data?.heading && <h3 className="text-2xl font-black font-heading text-slate-900 dark:text-white">{block.data.heading}</h3>}
                        <div className={`grid grid-cols-1 sm:grid-cols-${block.data?.columns || 3} gap-6`}>
                          {(block.data?.banners || []).map((b: any, bIdx: number) => (
                            <Link key={bIdx} href={b.url || '/collections'} className="relative rounded-3xl overflow-hidden aspect-4/3 group shadow-md block">
                              ...
                              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/25 to-transparent flex flex-col justify-end p-6 text-white">
                                {b.badge && <span className="self-start px-2.5 py-0.5 rounded-md bg-white text-slate-900 font-black text-[10px] uppercase mb-1.5">{b.badge}</span>}
                  ...
                  case 'instagram_feed': {
                    return (
                      <div key={block.id || idx} className="space-y-6 text-center">
                        <div>
                          <h3 className="text-2xl font-black font-heading text-slate-900 dark:text-white">{block.data?.heading || 'Follow Us'}</h3>
                          <p className="text-xs font-bold text-indigo-600 mt-1">{block.data?.handle || '@store_official'}</p>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                          {(block.data?.items || []).map((it: any, itIdx: number) => (
                            <div key={itIdx} className="relative rounded-2xl overflow-hidden aspect-square group shadow-xs">
                      ...
                      <div key={block.id || idx} className="space-y-8">
                        ...
                        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${block.data?.columns || 4} gap-5`}>
                          {(block.data?.features || []).map((feat: any, fIdx: number) => (
                            <div key={fIdx} className="p-6 rounded-2xl border border-slate-200/60 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm space-y-3 text-center hover:shadow-md transition">
                  ...
                  case 'image_text': {
                    ...
                    return (
                      <div key={block.id || idx} className={`flex flex-col ${isRight ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-10 p-8 rounded-3xl bg-slate-50 dark:bg-neutral-900 border border-slate-200/60 dark:border-neutral-800`}>
                        ...
                        <div className="w-full md:w-1/2 space-y-4">
                          ...
                          <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{block.data?.description || block.data?.text || ''}</p>
                          {block.data?.buttonText && (
                      ...
                      <div key={block.id || idx} className="space-y-6 max-w-3xl mx-auto">
                        ...
                        <div className="space-y-3">
                          {(block.data?.items || []).map((faqItem: any, faqIdx: number) => {
                            ...
                            return (
                              <div key={faqIdx} className="rounded-2xl border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden shadow-xs">
                                <button
                                  type="button"
                                  onClick={() => setOpenFaqIndex(isOpen ? null : (idx * 100 + faqIdx))}
                                  className="w-full px-6 py-4 text-left font-bold text-sm flex items-center justify-between gap-3 cursor-pointer hover:bg-slate-50 dark:hover:bg-neutral-800 transition"
                                >
                      ...
                      <div key={block.id || idx} className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          {(block.data?.reviews || []).map((rev: any, rIdx: number) => (
                            <div key={rIdx} className="p-6 rounded-3xl border border-slate-200/60 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm space-y-4 flex flex-col justify-between">
                              <div className="space-y-3">
                              ...
                              <div className="pt-4 border-t border-slate-100 dark:border-neutral-800">
                                <div className="font-bold text-sm">{rev.author || rev.name}</div>
                                <div className="text-xs text-slate-400">{rev.role || 'Verified Customer'}</div>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    return (
                      <div
                        ...
                        className={`p-10 sm:p-16 rounded-3xl text-center space-y-6 ${
                          block.data?.bgDark ? 'bg-slate-900 text-white' : 'bg-slate-50 dark:bg-neutral-900 border border-slate-200/60'
                        }`}
                      >
                        <div className="max-w-xl mx-auto space-y-2">
                          <h2 className="text-2xl sm:text-4xl font-extrabold font-heading">{block.data?.heading || 'Stay in the Loop'}</h2>
                          {block.data?.subtitle && <p className="text-sm opacity-80">{block.data.subtitle}</p>}
                        </div>
                        ...
                        {newsletterSubmitted ? (
                          ...
                          >
                            ...
                            <button
                              type="submit"
                              className="px-7 py-3.5 rounded-xl bg-white text-slate-950 font-bold text-sm shadow hover:bg-slate-100 transition"
                            >
                    ...
                    return (
                      ...
                      >
                        <div className="max-w-2xl mx-auto space-y-3 relative z-10">
                          <h2 className="text-3xl sm:text-5xl font-black font-heading">{block.data?.headline || 'Ready to Start?'}</h2>
                          {block.data?.subtitle && <p className="text-base sm:text-lg opacity-80">{block.data.subtitle}</p>}
                        </div>
                        <div className="flex flex-wrap items-center justify-center gap-4 relative z-10">
                          {block.data?.primaryButtonText && (
                            <Link href={block.data?.primaryButtonUrl || '/products'} className="px-8 py-3.5 rounded-xl bg-white text-slate-950 font-bold text-sm shadow hover:bg-slate-100 transition">
                              {block.data.primaryButtonText}
                            </Link>
                          )}
                          {block.data?.secondaryButtonText && (
                            <Link href={block.data?.secondaryButtonUrl || '/pages/contact'} className="px-8 py-3.5 rounded-xl bg-white/20 text-white font-bold text-sm backdrop-blur-sm hover:bg-white/30 transition">
                              {block.data.secondaryButtonText}
                            </Link>
                          )}
                        </div>
                  ...
                  case 'video':
                    return (
                      <div key={block.id || idx} className="space-y-4 max-w-4xl mx-auto">
                        {block.data?.heading && <h2 className="text-2xl sm:text-3xl font-bold font-heading text-center">{block.data.heading}</h2>}
                        <div className="relative aspect-video rounded-3xl overflow-hidden shadow-lg border border-slate-200 dark:border-neutral-800">
                          <iframe src={block.data?.videoUrl || 'https://www.youtube.com/embed/dQw4w9WgXcQ'} className="w-full h-full" allowFullScreen title={block.data?.heading || 'Video'} />
                        </div>
                      </div>
                    );
  ...
  return (
    <TemplateLayout theme={theme}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
        {page.title && (
          <header className="border-b border-slate-200/80 dark:border-neutral-800 pb-6">
